            ...
            cb =>{
                ...
                Models.users.findOne(queryObj).lean().exec((err, res)=>{
                    if(err){
                        ...
                        Models.users.findOneAndUpdate(queryObj,UserObj).lean().exec((err, resp)=>{
                            if (err) {
                                cb({statusCode: 400, status: "error", message: "Technical error. Please try again later."})
                            }else {
                        ...
                        // Add new user here.
                        Models.users(UserObj).save((err,resp)=> {
                            if (err) {
                                cb({statusCode: 400, status: "error", message: "Technical error. Please try again later."})
                            }else {
                                data.user_id = resp._id;
            ...
            (data, cb) =>{
                ...
                Models.products.findOne(queryObj).lean().exec((err, res)=>{
                    if(err){
                        ...
                        Models.products.findOneAndUpdate(queryObj,productObj).lean().exec((err, resp)=>{
                            if (err) {
                                cb({statusCode: 400, status: "error", message: "Technical error. Please try again later."})
                            }else {
                        ...
                        Models.products(productObj).save((err,resp)=> {
                            if (err) {
                                cb({statusCode: 400, status: "error", message: "Technical error. Please try again later."})
                            }else {
                                data.product_id = resp._id;
